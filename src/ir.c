...
identifier_t *add_var(character_t *name, scope_t scope, int idx)
{
	ensure_allocated();
...
int translate_recursive(buf_writer_t *writer, ast_node_t *node)
{
	...
	{
		case AST_IDENTIFIER:
			identifier_t *ident = get_variable(node->data.identifier);
			if (!ident)
			{
		...
		case AST_DECLARATION:
			...
			character_t *var_label =
			    add_var(node->data.declaration.identifier->data.identifier, LOCAL, vardecl_idx)
		...
		case AST_FUNCTION:
			...
			for (int i = 0; i < node->data.function.param_count; i++)
			{
				...
				add_var(arg_ident, ARG, func_arg_idx);
